...
int64 nChainStartTime = 1435887880;
...
{
	int64 nSubsidy = 0.668 * COIN;

	if(nHeight == 1)
	{
		nSubsidy = 23000000 * COIN;	// 100 millions coins, that all pow coins
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
	...
	if(nHeight < YEARLY_BLOCKCOUNT)
		nRewardCoinYear = 1.1 * MAX_MINT_PROOF_OF_STAKE;
	else if(nHeight < (2 * YEARLY_BLOCKCOUNT))
		nRewardCoinYear = 1.01 * MAX_MINT_PROOF_OF_STAKE;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in RhinoCoin
    bool fStrictPayToScriptHash = true; // Always active in RhinoCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "RhinoCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "July 20, 2015, RHC lanuched.!.";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1435887880;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 135774569;
        ...
        assert(block.hashMerkleRoot == uint256("0x0e056fb3460523ed594b1266b278c375fffc7cc1d7f75215bc5cddd3928152e6"));
